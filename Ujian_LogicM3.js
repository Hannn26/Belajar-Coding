...
function naikAngkot(arrPenumpang) {
    ...
    for(let i = 0; i < arrPenumpang.length;i++){
        ...
        for(let j = 0; j < rute.length;j++){
            if(receipt[i].naikDari == rute[j]){
                for(let k = 0; k < rute.length;k++){
                    if(receipt[i].tujuan == rute[k]){
                        receipt[i].bayar = (k - j) * 2000
                    }
                }
            }
        }
  ...
  function highestScore(students) {
    // Create an object to store the highest scores for each class
    let receipt = {};
    for(let i = 0; i < students.length;i++){
      if(receipt[students[i].class] == undefined){
        receipt[students[i].class] = {
          nama: students[i].name,
          score: students[i].score
        }
      }
      else if(students[i].score > receipt[students[i].class].score){
          receipt[students[i].class] = {
            nama: students[i].name,
            score: students[i].score
          }
  ...
  for(let i = 0; i < students.length;i++){
    if(students[i].score > 75){
      if(receipt[students[i].class] == undefined){
        ...
        receipt[students[i].class].push(data);
      }
      else{
        data = {
          name: students[i].name,
