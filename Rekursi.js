...
function sorting(arrNumber) {
    // code di sini
    let rey = arrNumber;
    rey.sort(function(a, b){return b - a});
    if(rey[rey.length - 1] < rey[0]){
        rey.pop();
        return sorting(rey);
    }
    else{
...
function changeVocals(str) {
  //code di sini
  let daftarHuruf = "aiueo"
  let perubahan = "bjvfp"
  for(let i = 0; i < str.length;i++){
    for(let j = 0; j < daftarHuruf.length;j++){
      if(str[i] == daftarHuruf[j]){
        str = str.replace(str[i],perubahan[j])
      }
      else if(str[i] == daftarHuruf[j].toUpperCase()){
        str = str.replace(str[i],perubahan[j].toUpperCase())
      }
    }
  }
  return str
...
function totalDigitRekursif(angka) {
  ...
  if(angkaStr.length == 1){
    ...
    let sliceAngka = angkaStr.slice(0,1)
    let newAngka = angkaStr.slice(1)
    return parseInt(sliceAngka) + totalDigitRekursif(parseInt(newAngka));
  }
}
...
Karena 24 dua digit, maka kita lakukan 2 * 4 = 8. 8 satu digit, maka hasilnya adalah 8. 
